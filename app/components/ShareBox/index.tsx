import { RiFacebookFill, RiPrinterLine } from '@remixicon/react';
...
export default function ShareBox() {
  ...
  const handlePrint = (e: any) => {
    e.preventDefault();
    const printContent = document.getElementById('article_detail')
      ?.outerHTML as string;
  ...
  return (
    <Hydrated>
      <section className='flex items-center justify-center p-4 rounded-lg bg-zinc-100 w-fit'>
        <h3 className='text-xl font-bold text-[--sub6-text-color]'>Chia sẻ</h3>

        <ul className='flex items-center border-l border-[color:--sub4-text-color] ml-4'>
          <li className='ml-4'>
          ...
          <li className='ml-4'>
            <a href='#' onClick={handlePrint}>
              <RiPrinterLine
                className='text-[color:--sub6-text-color]'
                size={24}
              />
            </a>
          </li>
          ...
          <li className='ml-4'>
            ...
            >
              <LinkIcon className='text-[color:--sub6-text-color]' size={24} />
            </a>
          </li>
        </ul>
      </section>
